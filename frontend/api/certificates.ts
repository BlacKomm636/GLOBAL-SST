...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const qrPath = `${folder}/${code}-qr.png`;
    const pdfPath = `${folder}/${code}.pdf`;
    ...
    const qrUrl = admin.storage.from('certificates').getPublicUrl(qrPath).data.publicUrl;
    ...
    if (insertError) {
      const { error: cleanupError } = await admin.storage.from('certificates').remove([qrPath, pdfPath]);
      if (cleanupError) {
        console.error('No se pudieron limpiar los objetos huerfanos en storage:', cleanupError);
      }
    ...
    console.error('Error inesperado emitiendo certificado:', err);
